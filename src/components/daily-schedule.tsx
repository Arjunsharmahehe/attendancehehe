...
export const DailySchedule = memo(function DailySchedule({
    ...
    if (dayName === "Weekend") {
        return (
            <text attributes={TextAttributes.DIM}>Weekend - No Classes</text>
        );
    }
    if (slots.length === 0) {
        return (
            <text attributes={TextAttributes.DIM}>No classes scheduled.</text>
        );
    }
    ...
    return (
        <box flexDirection="column">
            {slots.map((subId, idx) => (
                <DailyScheduleRow
                    key={idx} // Index is safe here as schedule structure is fixed per day
                    subId={subId}
