...
interface OverallStatsProps {
    subjects: Subject[];
    // We pass this trigger to force re-render when data changes,
    // but internally we use the hook or passed props.
    // To keep it pure, let's pass the calculated stats or the raw data needed.
    // Ideally, we use the hook inside, but for memoization to work against PARENT state changes,
    // we should rely on context or stable props.
}
        ...
        <box flexDirection="column">
            ...
            <box flexDirection="column" marginBottom={2}>
                ...
                <box flexDirection="row" gap={3} marginTop={1}>
                    ...
                    <text fg="green">
                        Buffer: {overallStats.totalSafeBunks}
                    </text>
                </box>
            </box>
            ...
            {subjectStats.map((stat) => (
                <box key={stat.id} flexDirection="row" height={1}>
                    ...
                    <text
                        width={8}
                        fg={stat.percentage >= 75 ? "green" : "red"}
                    >
